...
void Si4703_Breakout::setChannel(int channels)
{
  ...
  chn = channels - 875;  //this is valid for Europe

  //These steps come from AN230 page 20 rev 0.5
  readRegisters();
  si4703_registers[CHANNEL] &= 0xFC00; //Clear out the channel bits
  si4703_registers[CHANNEL] |= chn; //Mask in the new channel
...
void Si4703_Breakout::si4703_init() 
{
  ...
  readRegisters(); //Read the current register set
  si4703_registers[POWERCFG] = 0xC101; //Enable the IC, enable seek.
  //  si4703_registers[POWERCFG] |= (1<<SMUTE) | (1<<DMUTE); //Disable Mute, disable softmute
  si4703_registers[SYSCONFIG1] = 0x0800; //enable RDS and de-emphasis = 50 uSec. 
...
//  si4703_registers[SYSCONFIG2] |= (1<<SPACE0); //100kHz channel spacing for Europe
  ...
  si4703_registers[SYSCONFIG2] = 0x0011; 
...
int Si4703_Breakout::getChannel() {
  readRegisters();
  int channelr = si4703_registers[READCHAN] & 0x01FF; //Mask out everything but the lower 10 bits
